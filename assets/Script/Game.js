cc.Class({
    ...
    spawNewCoin: function(pos,rl) {
        ...
        let a = newCoin.getComponents('Coin');
        for(var obj of a)
            obj.game = this;
    },
    ...
    spawNewObstacles: function(pos,rl) {
        ...
        let a = newObstacles.getComponents('Obstacles');
        for(var obj of a)
            obj.game = this;
    },
    ...
    backgroundChange: function(time){
        ...
        this.bg1.y -= s;
        this.bg2.y -= s;
        if (this.cur_bg.y <= -this.m_size.height) 
        { // 地图切换
    ...
    flashUpdate: function(){
        ...
        if(this.m_player.untouchCd > 0)
            this.m_player.untouchCd--;
        else
            this.m_player.untouch = false;
    },
    ...
    update: function (dt) {
        ...
        if(this.makeobj % this.makeobjCd == 0)
            this.makeObject(dt);
        if(this.scoretime % this.mScoreTime == 0)
            this.gainScore();
